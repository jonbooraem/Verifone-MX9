...
namespace Verifone_MX9.Tests
{
    class IOE2
    {
        ...
        public IOE2(Form1 form_1, int port)
        {
        ...
        {
            form.updatePanelColor((portNum == 1) ? (form.IOEResult) : (form.IOEResult2), Color.Yellow);
            form.AppendText(form.logs, System.Drawing.Color.Blue, "Starting IOE Test \n");
            form.testLog += "Starting IOE Test\n";
            serial.output = "";

            for (int i = 0; i < Form1.tests.Length; i++)
            {

                if (Form1.tests[i].Contains("IOE"))
                    Check_Results.Set_Progress(i, form.testTable, form);
            }


            //send the command
            serial.Write_Serial(probe_port.PortName, "test ioe");

            //call robot
            //Form1.writeAds("Trigger1", true);

            int timeout;
            if (ConfigurationManager.AppSettings.Get("IOETimeout").Equals("NONE")) timeout = -1;
            else timeout = Int32.Parse(ConfigurationManager.AppSettings.Get("IOETimeout"));
            ...
            while (!(serial.output.Contains("FAIL") || serial.output.Contains("PASS")) && !serial.stopped)
            {
                if (form.stopTest)
                {
                    for (int i = 0; i < Form1.tests.Length; i++)
                    {
                        ...
                        if (Form1.tests[i].Contains("IOE"))
                        {
                            form.updatePanelColor((portNum == 1) ? (form.IOEResult) : (form.IOEResult2), Color.Red);
