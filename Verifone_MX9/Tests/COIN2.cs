...
namespace Verifone_MX9.Tests
{
    class COIN2
    {
        ...
        public COIN2(Form1 form_1, int port)
        {
        ...
        {
            form.updatePanelColor((portNum == 1) ? (form.COINResult) : (form.COINResult2), Color.Yellow);
            serial.output = "";
            form.AppendText(form.logs, System.Drawing.Color.Blue, "Starting Coin Test \n");
            form.testLog += "Starting Coin Test\n";
            for (int i = 0; i < Form1.tests.Length; i++)
            {

                if (Form1.tests[i].Contains("COIN"))
                    Check_Results.Set_Progress(i, form.testTable, form);
            }

            probe_port.Open();
            probe_port.WriteLine("test coin");
            Thread.Sleep(500);
            ...
            int timeout;
            if (ConfigurationManager.AppSettings.Get("COINTimeout").Equals("NONE")) timeout = -1;
            else timeout = Int32.Parse(ConfigurationManager.AppSettings.Get("COINTimeout"));
            ...
            while (!(serial.output.Contains("FAIL") || serial.output.Contains("PASS")) && !serial.stopped)
            {
                if (form.stopTest)
                {
                    for (int i = 0; i < Form1.tests.Length; i++)
                    {
                        ...
                        if (Form1.tests[i].Contains("COIN"))
                        {
                            form.updatePanelColor((portNum == 1) ? (form.COINResult) : (form.COINResult2), Color.Red);
