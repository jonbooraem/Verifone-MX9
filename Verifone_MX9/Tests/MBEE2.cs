...
namespace Verifone_MX9.Tests
{
...

    {
        ...
        public MBEE2(Form1 form_1, int port)
        {
        ...
        {
            form.updatePanelColor((portNum == 1) ? (form.MBEEResult) : (form.MBEEResult2), Color.Yellow);
            form.AppendText(form.logs, System.Drawing.Color.Blue, "Starting MBEE Test \n");
            form.testLog += "Starting MBEE Test\n";
            serial.output = "";

            for (int i = 0; i < Form1.tests.Length; i++)
            {

                if (Form1.tests[i].Contains("MBEE"))
                    Check_Results.Set_Progress(i, form.testTable, form);
            }


            //send the command
            serial.Write_Serial(probe_port.PortName, "test mbee");

            //call robot
            //Form1.writeAds("Trigger1", true);

            int timeout;
            if (ConfigurationManager.AppSettings.Get("MBEETimeout").Equals("NONE")) timeout = -1;
            else timeout = Int32.Parse(ConfigurationManager.AppSettings.Get("MBEETimeout"));
            ...
            while (!(serial.output.Contains("FAIL") || serial.output.Contains("PASS")) && !serial.stopped)
            {
                if (form.stopTest)
                {
                    for (int i = 0; i < Form1.tests.Length; i++)
                    {
                        ...
                        if (Form1.tests[i].Contains("MBEE"))
                        {
                            form.updatePanelColor((portNum == 1) ? (form.MBEEResult) : (form.MBEEResult2), Color.Red);
