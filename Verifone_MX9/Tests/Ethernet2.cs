...
namespace Verifone_MX9.Tests
{
...

    {
        ...
        public Ethernet2(Form1 form_1, int port)
        {
        ...
        {
            form.updatePanelColor((portNum == 1) ? (form.LANResult) : (form.LANResult2), Color.Yellow);
            serial.output = "";
            form.AppendText(form.logs, System.Drawing.Color.Blue, "Starting LAN Test \n");
            form.testLog += "Starting LAN Test\n";
            for (int i = 0; i < Form1.tests.Length; i++)
            {

                if (Form1.tests[i].Contains("LAN"))
                    Check_Results.Set_Progress(i, form.testTable, form);
            }

            probe_port.Open();
            probe_port.WriteLine("test lan");
            Thread.Sleep(500);
            ...
            int timeout;
            if (ConfigurationManager.AppSettings.Get("EthernetTimeout").Equals("NONE")) timeout = -1;
            else timeout = Int32.Parse(ConfigurationManager.AppSettings.Get("EthernetTimeout"));
            ...
            while (!(serial.output.Contains("FAIL") || serial.output.Contains("PASS")) && !serial.stopped)
            {
                if (form.stopTest)
                {
                    for (int i = 0; i < Form1.tests.Length; i++)
                    {
                        ...
                        if (Form1.tests[i].Contains("LAN"))
                        {
                            form.updatePanelColor((portNum == 1) ? (form.LANResult) : (form.LANResult2), Color.Red);
