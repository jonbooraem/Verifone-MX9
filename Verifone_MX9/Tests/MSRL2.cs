...
namespace Verifone_MX9.Tests
{
    class MSRL2
    {
        ...
        public MSRL2(Form1 form_1, int port)
        {
        ...
        {
            form.updatePanelColor((portNum == 1) ? (form.MSRLResult) : (form.MSRLResult2), Color.Yellow);
            serial.output = "";
            form.AppendText(form.logs, System.Drawing.Color.Blue, "Starting Magnetic Strip Reader Low Test \n");
            form.testLog += "Starting Magnetic Strip Reader Low Test\n";
            for (int i = 0; i < Form1.tests.Length; i++)
            {

                if (Form1.tests[i].Contains("MSRL"))
            ...
            while (!(serial.output.Contains("FAIL") || serial.output.Contains("PASS")) && !serial.stopped)
            {
                if (form.stopTest)
                {
                    for (int i = 0; i < Form1.tests.Length; i++)
                    {
                        ...
                        if (Form1.tests[i].Contains("MSRL"))
                        {
                            form.updatePanelColor((portNum == 1) ? (form.MSRLResult) : (form.MSRLResult2), Color.Red);
