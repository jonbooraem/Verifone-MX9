...
namespace Verifone_MX9.Tests
{
    class LED2
    {
        ...
        public LED2(Form1 form_1, int port)
        {
        ...
        {
            form.updatePanelColor((portNum == 1) ? (form.LEDResult) : (form.LEDResult2), Color.Yellow);
            serial.output = "";
            form.AppendText(form.logs, System.Drawing.Color.Blue, "Starting LED Test \n");
            form.testLog += "Starting LED Test\n";
            for (int i = 0; i < Form1.tests.Length; i++)
            {

                if (Form1.tests[i].Contains("LED"))
                    Check_Results.Set_Progress(i, form.testTable, form);
            }

            //send the command
            serial.Write_Serial(probe_port.PortName, "test led");

            //call robot
            //Form1.writeAds("Trigger1", true);

            int timeout;
            if (ConfigurationManager.AppSettings.Get("LEDTimeout").Equals("NONE")) timeout = -1;
            else timeout = Int32.Parse(ConfigurationManager.AppSettings.Get("LEDTimeout"));
            ...
            while (!(serial.output.Contains("FAIL") || serial.output.Contains("PASS")) && !serial.stopped)
            {
                if (form.stopTest)
                {
                    for (int i = 0; i < Form1.tests.Length; i++)
                    {
                        ...
                        if (Form1.tests[i].Contains("LED"))
                        {
                            form.updatePanelColor((portNum == 1) ? (form.LEDResult) : (form.LEDResult2), Color.Red);
